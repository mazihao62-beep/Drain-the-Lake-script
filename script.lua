...
local LANG = {
    Chinese = {
        ...
        waterStart = "开始无限拿水",
        waterStop = "停止无限拿水",
        interactStart = "开启全图互动+秒互动",
        interactStop = "关闭全图互动+秒互动",
        sell1 = "自动售卖(1号机)",
        sell1Stop = "停止售卖(1号机)",
        chest = "自动开箱子",
        chestStop = "停止开箱子",
    ...
    English = {
        ...
        waterStart = "Start Infinite Water",
        waterStop = "Stop Infinite Water",
        interactStart = "Enable Full Map Interaction",
        interactStop = "Disable Full Map Interaction",
        sell1 = "Auto Sell (Machine 1)",
        sell1Stop = "Stop Sell (Machine 1)",
        chest = "Auto Open Chest",
        chestStop = "Stop Open Chest",
...
function createMainGui()
    ...
    local function addToggleHover(btn, getIsActiveFunc, activeColor)
        ...
        btn.MouseEnter:Connect(function()
            if not getIsActiveFunc() then
                btn.BackgroundColor3 = orig:Lerp(Color3.new(1,1,1), 0.15)
            end
        end)
        btn.MouseLeave:Connect(function()
            if getIsActiveFunc() and activeColor then
    ...
    local function modifyPrompt(prompt, enable)
        if enable then
            if not originalData[prompt] then
                originalData[prompt] = {
            ...
            if data and prompt and prompt.Parent then
                prompt.MaxActivationDistance = data.Distance
    ...
    interactBtn.MouseButton1Click:Connect(function()
        ...
        if isInteractActive then
            interactBtn.Text = lang.interactStop
            ...
            for prompt, data in pairs(originalData) do
